...
function getRow(keys, rows, rowIndex, key) {
  let row;
  ...
  if (index !== -1) {
    ...
    keys.splice(rowIndex, 0, key);
  }
  return row;
}

function fillInCell(locales, keys, rows, value, columnIndex, rowIndex, key) {
  let row = getRow(keys, rows, rowIndex, key);
    ...
    function recurse(json, columnIndex, rowIndex, currentKey) {
      for (let key in json) {
        ...
        if (typeof value === 'object') {
          ...
          fillInCell(locales, keys, rows, value, columnIndex, rowIndex, newKey);
          rowIndex++;
        }
      }
      return rowIndex;
